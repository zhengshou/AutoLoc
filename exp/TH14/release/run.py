...
cmd1 = 'python tools/proc_net.py --phase train --dataset TH14 --expname '+EXP_NAME+' --rsltname '\
+'"tmp"'
os.system(cmd1)

# test & eval on all testing videos. prll mode.
rewrite_cfg_str('DATA_FILE','att_unfused\/test.all.tsv')
rewrite_cfg_str('STAGE','test')
rewrite_cfg('MAX_ITER',MAX_ITER)
cmd2 = 'python tools/proc_prll_net.py --phase test --dataset TH14 --expname '+EXP_NAME+' --rsltname '\
...
os.system(cmd2)
# single nms eval
cmd3 = 'python tools/eval_metrics.py --phase test --dataset TH14 --expname '+EXP_NAME+' --rsltname '\
+'"test_rslt'+'_trainnms'+str(0.4)+'_fg'+str(0.1)+'_ioc'+str(0.7)\
+'_testnms'+str(0.4)+'"'+' --num_workers '+str(num_process)
os.system(cmd3)
